...
- Primal Problem :: 
    ...
    - If $x$ is feasible, then the $\inf$ is zero. If $x$ is infeasible, then at least one constraint is violated and we can choose $y_i$ to make the $\inf = -\infty$. 
\[L_P(x) = \begin{cases} f(x) & \mbox{if $x$ is feasible}  \\ f(x) - (-\infty) = \infty & \mbox{if $x$ is not feasible}
\end{cases}\] 
