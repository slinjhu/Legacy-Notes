...
* Design Pattern
** TODO OO Principles ::
- Encapsulate what varies :: Identify the aspects of your application that vary, and separate them from what stays the same, so that later you can alter or extend the parts that vary without affecting those that don't. All design patterns provide a way to let some part of a system vary independently of all other parts. 
- Program to interfaces, not implementations :: The declared type of the variables should be a supertype (Java interface or abstract class), so that the objects assigned to those variables can be of any concrete implementation of the supertype, which means the class declaring them doesn't have to know about the actual object types. 
#+INCLUDE: "./source/DesignPattern/DemoInterface.java" src java
- Favor composition over inheritance :: Has-a relationship can be better than Is-a relationship since it is more flexible. 
With composition, objects can dynamically change behavior at runtime; with inheritance, objects are fixed after compilation.
- Loose couple :: Strive for loosely coupled designs between objects that interact
...
- Principle of Least Knowledge :: reduce the interactions between objects to just a few close friends.
- The Hollywood Principle :: Don't call us; we'll call you. We allow low-level components to hook themselves into a system, but the high-level components determines when and how they are needed. 
...
- Decorators have the same super type ad the objects they decorate, and you can use one or more decorators to wrap the object.
...
Definition: the time rate of change for a material element subjected to a space-and-time dependent macroscopic velocity field. 
\[
  \dd{f(\vx,t)}{t} =
  \pp{f}{t} + \pp{f}{\vx} \cdot \pp{\vx}{t}
  = \pp{f}{t} + \vu\cdot \nabla f
\]
where, not rigorously, 
\[
  \nabla f = \pp{f}{\vx}
  \mbox{ and } 
  \vu = \pp{\vx}{t}
...
- Definition: the rate of change over time of local deformation

*** Dimensionless numbers
**** Peclet number
**** Reynolds number
- defined as $Re = \rho U L / \mu$
- where the parameters are defined as 
    + $\rho$ : density of the fluid
    + $U$ : characteristic velocity.
    + $L$ : representative length scale. 
    + $\mu$: Dynamic viscosity of fluid. (1e-3 Pa.s for water at 20C)
- meaning of Re:
    + $Re < 2300$: laminar flow
    + $Re > 4000$: turbulent flow.
    + Re = (inertial forces) / (viscous forces)
**** Knudsen number
- defined as $Kn = \lambda / L$
- parameters
    + $\lambda$: length of the mean free path of the molecules. 
    + $L$: representative length scale
- meaning of Kn
    + $Kn < 0.01$: NS equations are valid. 
    + $0.01 < Kn < 0.1$: NS equations can still be used but with special boundary conditions. 
    + $Kn > 0.1$: NS equations are not valid. 
...
- Singularity :: Let $A$ be a square matrix, then the followings are equivalent: 
    ...
    - Determinant $|A| \ne 0$
    - $A$ has an inverse. 
- Orthogonal :: A square matrix $Q$ is orthogonal if $Q^T Q = Q Q^T=I$. $Q^{-1} = Q^T$
...
For two n-digit number $X$ and $Y$, and $m = n/2$, 
\[ X = x_1 \cdot 2^m + x_2 \]
\[ Y = y_1 \cdot 2^m + y_2 \] 
then we have 
\[ X \cdot Y = 2^{2m} \cdot (x_1 y_1) + 2^m \cdot (x_1 y_2 + y_1 x_2) + x_2 y_2 \] 
The basic idea of Karatsuba algorithm is to reduce number of multiplications in the above recursive equation, using the following equation 
\[ x_1 y_1 + x_2 y_1 + x_1 y_2 + x_2 y_1 = (x_1 + x_2) (y_1 + y_2) \]

**** Strassen matrix multiplication
2D version of Karatsuba multiplication. The basic idea is also to reduce number of multiplications.

*** Sorting
-  *Selection sort*: find the smallest currently unsorted element, put it just after the set of sorted elements, and repeat.
-  *Merge-sort*: recursively sort the left half of the array, then the right half, and then merge the two sorted halves.
-  *Quick-sort; $\Theta(n^2)$*: choose an element $p$ as pivot, compare each element with $p$, make an array consisting of $L$ (elements less than $p$), followed by $p$ and then followed by $G$ (elements greater than $p$), and recursively sort $L$ and $G$.
-  *Randomized Quick-sort; $\Theta(n\log n)$*: choose pivot randomly.
-  *Radix sort; $O(n)$*: use bucket sort on each digit.

*** Selection
-  *Randomized Quick-select*: similar to quick-sort but only recurse on the side which has the desired element.

** Dynamic Programming (DP)
*** Ideas
-  Multi-stage optimization: the decisions we make at some point are influenced by the decisions we made earlier.
-  Programming in here means a way of tabulation.
-  Similar to Divide-and-Conquer but with overlapping subproblems.

**** Elements of dynamic programming
Two key ingredients that an optimization problem must have in order for DP to apply.
-  *Optimal substructure*: an optimal solution to the problem contains within it optimal solutions to subproblems.
-  Subproblems must be independent. The solution to one subproblem does not affect the solution to another subproblem of the same problem. The subproblems should not share resources.
-  *Overlapping subproblems*: the space of subproblems must be small in the sense that a recursive algorithm solve the same subproblems over and over, rather than always generating new subproblems.
-  DP takes advantages of overlapping subproblems by solving each subproblem once and storing the solution in a table.
...
Let $X, Y$ be sequences and $X$ be a LCS of $X$ and $Y$.
-  If $x_m = y_n$, then $z_k = x_m = y_n$ and $Z_{k-1}$ is an LCS of $X_{m-1}$ and $Y_{n-1}$.
-  IF $x_m\ne y_n$, then $z_k\ne x_m$ implies that $Z$ is LCS of $X_{m-1}$ and $Y$.
-  IF $x_m\ne y_n$, then $z_k\ne y_n$ implies that $Z$ is LCS of $X$ and $Y_{n-1}$.
...
***** Optimal substructure
To determine the optimal BST is to determine the root in each level. Let $k_r$ be the root of optimal BST containing $(k_1, k_2, \dots, k_n)$, then the left subtree containing $(k_1, k_2, \dots, k_{r-1})$ and the right subtree $(k_{r+1}, \dots, k_n)$ must also be optimal.
...
*** Definitions
-  *rooted tree*: a tree with one vertex $r$ chosen as root.
-  *$P(v)$*: the unique $vr$ -path.
-  *parent* of $v$: neighbor of $v$ on $P(v)$.
-  *children* of $v$: its other neighbors other than its parent.
-  *ancestors* of $v$: the vertices in $P(v)-v$.
-  *descendants* of $v$: all the vertices $u$ such that $v\in P(u)$.
-  *leaves*: vertices with on children.
-  *rooted plane tree* or *planted tree*: an ordered tree.
-  *binary tree*: a rooted plane tree where each vertex has at most two children, and each its child is designated as *left child* or *right child*.
...
int main(){
  ...
  // Unnamed temporary with braced-init-list
  for(auto x: {1,2,3}){
    cout << x << " ";
  }
}
